    ...
    public static class OperatorUtils {
        ...
        public static ColumnOperator<T> And<T>(IEnumerable<ColumnOperator<T>> values) {
            ...
            switch (operators.Count()) {
                ...
                default:
                    return new AndOperator<T>(values);
            }
        }
        ...
        public static ColumnOperator<T> Or<T>(IEnumerable<ColumnOperator<T>> values) {
            ...
            switch (operators.Count()) {
                ...
                case 1:
                    return operators.Single();
                default:
                    return new OrOperator<T>(values);
            }
        }
        ...
        private sealed class EqualsOperator<T> : ColumnOperator<T> {
            ...
            public override WhereInfo CreateWhereInfoInternal(string columnName, string parameterName) {
                string clause = "";
                List<WhereInfo.ParameterInfo> parameters = new List<WhereInfo.ParameterInfo>();
                if (Values?.Any() == true) {
        ...
        private sealed class AndOperator<T> : ColumnOperator<T> {
            private readonly IEnumerable<ColumnOperator<T>> columnOperators;

            public AndOperator(IEnumerable<ColumnOperator<T>> columnOperators) {
                this.columnOperators = columnOperators.Where(x => x != null);
            }
        ...
        private sealed class OrOperator<T> : ColumnOperator<T> {
            private readonly IEnumerable<ColumnOperator<T>> columnOperators;

            public OrOperator(IEnumerable<ColumnOperator<T>> columnOperators) {
                this.columnOperators = columnOperators.Where(x => x != null);
            }
